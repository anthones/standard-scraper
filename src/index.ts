...
async function main() {
  while (linksToVisit.length > 0) {
    try {
      ...
      if (currentUrl && visitedLinks.includes(currentUrl)) continue;
      console.log("now crawling " + currentUrl);

      const html = await getRequest(
        "https://www.amazon.com" + currentUrl
      );

      const parsedResult = parseAll(html);

      const cleanLinks = parsedResult.productLinks.map((link: string) =>
        link.slice(0, 14)
      );

      linksToVisit = [...linksToVisit, ...cleanLinks];
      console.log(parsedResult);
      currentUrl && visitedLinks.push(currentUrl);
      await sleep(5000);
    } catch (err) {
      console.error(err);
    }
